...
export async function generateMetadata({ params }: BlogCategoryPageProps): Promise<Metadata> {
  ...
  if (!category) {
    return {
      title: "Category Not Found | Metro Mellow Blog",
    };
  }

  return {
    title: `${category.name} Tips & Guides | Metro Mellow Blog`,
    description: `${category.description}. Expert ${category.name.toLowerCase()} advice for Lagos homes from Metro Mellow's professional team.`,
    keywords: `${category.name.toLowerCase()} Lagos, ${category.name.toLowerCase()} tips Nigeria, home ${category.name.toLowerCase()}, professional ${category.name.toLowerCase()} services, Metro Mellow ${category.name.toLowerCase()}`,
    alternates: {
      canonical: `https://metromellow.com/blog/category/${category.slug}`,
    },
    openGraph: {
      title: `${category.name} Expert Tips | Metro Mellow Blog`,
...
export default async function BlogCategoryPage({ params }: BlogCategoryPageProps) {
  ...
  const breadcrumbs = [
    ...
    { name: category.name, url: `https://metromellow.com/blog/category/${category.slug}` },
  ];
        ...
        <section className={styles.header}>
          <div className={styles.container}>
            ...
            <h1 className={styles.title}>
              {category.name} Tips & Guides
            </h1>
            
            <p className={styles.description}>
              {category.description}. Get expert advice from our professional team 
              serving Lagos homes and businesses.
            </p>
              ...
              {categories
                ...
                .map((otherCategory) => (
                  <Link
                    ...
                    style={{ '--category-color': otherCategory.color } as React.CSSProperties}
                  >
                    <span className={styles.linkIcon}>{otherCategory.icon}</span>
                    <span className={styles.linkName}>{otherCategory.name}</span>
