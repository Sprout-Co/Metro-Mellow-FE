...
const ServiceEditor: React.FC<ServiceEditorProps> = ({
  ...
  return (
    <div
      className={`${styles.editor} ${accordionMode ? styles.editor__accordion : ""}`}
    >
      <div className={styles.editor__header}>
        <h2 className={styles.editor__title}>{title}</h2>
        {!accordionMode && (
          <button className={styles.editor__close} onClick={onClose}>
            ×
          </button>
        )}
      </div>
            ...
            {activeStep === 2 &&
              (serviceType === "cleaning" ? (
                <PropertySelector
                  ...
                  onHouseTypeChange={(type) =>
                    updateServiceData({ houseType: type })
                  }
                ...
                <ScheduleSelector
                  ...
                  onFrequencyChange={(freq) =>
                    updateServiceData({ deliveryFrequency: freq })
                  }
                  onDaysChange={(days) =>
                    updateServiceData({ deliveryDays: days })
                  }
                />
              ) : (
                <QuantitySelector
            ...
            {activeStep === 3 &&
              (serviceType === "cleaning" ? (
                <ScheduleSelector
                  ...
                  onFrequencyChange={(freq) =>
                    updateServiceData({ frequency: freq })
                  }
                ...
                <QuantitySelector
                  ...
                  days={serviceData.deliveryDays}
                  onMealsChange={(mealsPerDay) =>
                    updateServiceData({ mealsPerDay })
                  }
                />
              ) : (
                <ScheduleSelector
                  ...
                  onFrequencyChange={(freq) =>
                    updateServiceData({ pickupFrequency: freq })
                  }
                  onDaysChange={(days) =>
                    updateServiceData({ pickupDay: days[0] })
                  }
                />
              ))}
      ...
      <div className={styles.editor__actions}>
        {activeStep > 1 && (
          <button className={styles.editor__back_btn} onClick={handleBack}>
            ← BACK
          </button>
        )}

        <button className={styles.editor__continue_btn} onClick={handleNext}>
          {getButtonLabel()}
        </button>
      </div>
    </div>
