...
export const validateServiceConfiguration = (
  ...
  if (!configuration.frequency) {
    errors.push({ field: "frequency", message: "Service frequency is required" });
  }
  ...
  if (service.category === ServiceCategory.Cleaning) {
    if (!configuration.serviceDetails.cleaning) {
      errors.push({ field: "cleaning", message: "Cleaning details are required" });
    } else {
      const cleaning = configuration.serviceDetails.cleaning;
      
      if (!cleaning.houseType) {
        errors.push({ field: "houseType", message: "Property type is required" });
      }

      if (!cleaning.cleaningType) {
        errors.push({ field: "cleaningType", message: "Cleaning type is required" });
      }

      if (!cleaning.rooms) {
        errors.push({ field: "rooms", message: "Room configuration is required" });
      } else {
        const totalRooms = Object.values(cleaning.rooms).reduce((sum, count) => sum + (count || 0), 0);
        if (totalRooms === 0) {
  ...
  if (service.category === ServiceCategory.Cooking) {
    if (!configuration.serviceDetails.cooking) {
      errors.push({ field: "cooking", message: "Meal service details are required" });
    } else {
      ...
      if (!cooking.mealsPerDelivery || cooking.mealsPerDelivery.length === 0) {
        errors.push({ field: "mealsPerDelivery", message: "At least one meal delivery must be configured" });
      } else {
        const totalMeals = cooking.mealsPerDelivery.reduce((sum, delivery) => sum + (delivery.count || 0), 0);
        if (totalMeals === 0) {
          errors.push({ field: "mealsPerDelivery", message: "At least one meal must be configured" });
        }
      }
    }
  }
  ...
  if (service.options && service.options.length > 0) {
    if (!configuration.serviceDetails.serviceOption) {
      errors.push({ field: "serviceOption", message: "Service package is required" });
    } else {
      const validOption = service.options.find(opt => opt.id === configuration.serviceDetails.serviceOption);
      if (!validOption) {
        errors.push({ field: "serviceOption", message: "Invalid service package selected" });
      }
    }
  }
...
export const validateSubscription = (
  configuredServices: { service: Service; configuration: SubscriptionServiceInput }[],
  billingCycle: string,
  duration: number
): ValidationResult => {
  const errors: ValidationError[] = [];
  ...
  const billingValidation = validateBillingConfiguration(billingCycle, duration);
