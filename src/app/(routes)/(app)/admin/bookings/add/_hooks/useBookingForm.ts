...
interface BookingFormHandlers {
  ...
  setPropertyType: (type: HouseType) => void;
  updateRoomQuantity: (
    room: keyof RoomQuantitiesInput,
    increment: boolean
  ) => void;
  setLaundryBags: (bags: number) => void;
...
export const useBookingForm = (): [BookingFormState, BookingFormHandlers] => {
  ...
  const handlers: BookingFormHandlers = {
