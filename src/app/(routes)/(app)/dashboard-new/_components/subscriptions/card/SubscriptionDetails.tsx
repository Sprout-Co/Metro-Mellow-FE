...
const getIconByService = (
  ...
  return iconMap[service_category] || (fallback ?? "Sparkles");
};
...
export default function SubscriptionDetails({
  ...
  const serviceIcon = getIconByService(service_category, icon);
  const IconComponent = LucideIcons[serviceIcon] as
    | React.ElementType
    | undefined;
    ...
    <div className={styles.details}>
      <div className={styles.details__headerRow}>
        <div className={styles.details__left}>
          <div
            className={`${styles.details__icon} ${styles[`details__icon--${status}`]}`}
          >
            {IconComponent ? (
              <IconComponent className={styles.details__serviceIcon} />
            ) : null}
          </div>
          <div className={styles.details__titleWrap}>
            <h4 className={styles.details__title}>{service_category}</h4>
            <div className={styles.details__freqRow}>
              {frequency.map((freq, i) => (
                <span key={i} className={styles.details__freq}>
                  • {freq}
                </span>
              ))}
            </div>
        ...
        <div className={styles.details__showLess}>
          <Button
            variant="ghost"
            size="sm"
            className={styles.details__toggleButton}
            onClick={onShowLess}
          >
            SHOW LESS ▲
          </Button>
      ...
      <div
        className={styles.details__grid}
        role="group"
      ...
      >
        <div className={styles.details__col}>
          <div className={styles.details__field}>
            ...
            <span
              className={`${styles.details__status} ${styles[`details__status--${status}`]}`}
            >
              {status === "active"
                ? "Active"
      ...
      {items.length > 0 && (
        <div className={styles.details__items}>
          {items.map((item, idx) => (
            <div
              key={`${item.name}-${idx}`}
              className={styles.details__itemRow}
            >
              <div className={styles.details__itemLeft}>
                {(() => {
                  const itemIconName = getIconByService(item.name, icon);
                  const ItemIcon =
                    (LucideIcons[itemIconName] as React.ElementType) ||
                    undefined;
                  return (
                    <div
                      className={`${styles.details__miniIcon} ${styles[`details__miniIcon--variant-${(idx % 3) + 1}`]}`}
                    >
                      {ItemIcon ? (
                        <ItemIcon className={styles.details__miniIconIcon} />
                      ) : null}
                    </div>
                  );
                })()}
                <div className={styles.details__itemText}>
                  ...
                  <div className={styles.details__itemFreqRow}>
                    {item.frequency.map((f, i) => (
                      <span key={i} className={styles.details__itemFreq}>
                        • {f}
                      </span>
                    ))}
                  </div>
              ...
              <div className={styles.details__itemRight}>
                ...
                <div className={styles.details__itemPrice}>
      ...
      <div className={styles.details__actions}>
        ...
        <Button
          variant="secondary"
          className={styles.details__pause}
          onClick={onPause}
        >
          PAUSE
        </Button>
