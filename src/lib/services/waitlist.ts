...
class WaitlistService {
  ...
  async addToWaitlist(email: string, additionalData?: Partial<WaitlistEntry>): Promise<{ success: boolean; message: string; id?: string }> {
    try {
      ...
      if (existingUser) {
        return { success: false, message: 'You\'re already on our waitlist! We\'ll keep you updated.' };
      }
      ...
      const waitlistData: Omit<WaitlistEntry, 'id'> = {
        ...
        source: 'welcome_page',
        location: 'Nigeria', // Default for Metro Mellow
        createdAt: serverTimestamp(),
        ...additionalData
      };

      const docRef = await addDoc(collection(db, this.collectionName), waitlistData);
      
      return { 
        success: true, 
        message: 'Welcome to Metro Mellow! You\'re now on our exclusive waitlist.',
        id: docRef.id 
      };
